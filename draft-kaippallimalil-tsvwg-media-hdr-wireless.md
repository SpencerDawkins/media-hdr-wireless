...
Metadata defined in Section 4.2 is broad enough to be applied regardless of whether the application uses RTP, HTTP or another application protocol that uses UDP transport. The wireless network only inspects packets for which a rule to inspect metadata is present. Other network entities on-path do not process the UDP option since there is no configured rule.
